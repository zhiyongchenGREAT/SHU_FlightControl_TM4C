...
#define MAX_AXES 3
uint32 lastHighTime=0, lastLowTime=0,thisTime=0;
float accum_sin, accum_cos;
 ...
 * i.e. period and gain
 */
int stabilization_relay_rate(float error, float *output, int axis, _Bool reinit)
{
...
///////////timer change/////////////////// 
//        thisTime=timestampget(&timer);                                          //::timer::
       
        OS_ERR err;
        thisTime=OSTimeGet(&err);
/////////////////////////////////////////
	...
	*output = high ? relaySettings.Amplitude : -relaySettings.Amplitude;
	...
	_Bool time_hysteresis = (high ? (thisTime - lastHighTime) : (thisTime - lastLowTime)) > DEGLITCH_TIME;
